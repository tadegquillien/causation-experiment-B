...
const UrnMaker = () => {
  ...
  var possibleWorld = Array(8).fill(0);

  possibleWorld = urn_ids.map((i) => {
    let color = colors[i-1];
  ...
  const image = () => urn_ids.map((i) => {
    ...
    return(
        ...
        <svg height="300" width="300" id={i}  >
        <rect style={urn_style}></rect>
        {urnContents[i-1]}
        <circle cx="150" cy="200" r={r} fill={color} stroke="black" />
        </svg>
...
//
  ...
  return(
    <div>
      <p>
        The score is: {score}  .
      The minimum number of points to win was: {threshold}  .
      {win}</p>
      {urns}
    </div>
